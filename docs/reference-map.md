...
- `Get TOC` now exposes a deterministic `0x00CC`-word single-data-track TOC through the host data port at `0x25898000`, including first-track, empty-track, A0/A1, and leadout entries.
...
- The latest NiGHTS transform-source probe shows `0x0605D000..0x0605EFFF` is populated by the routine around `0x0602DC0C` with small signed fixed-point-looking values, then read by the transform builder around `0x0602E680..0x0602E7F0`. The first large transform writes use `R7=0x060300D4`, so the suspicious MAC coefficient/source path is the transform-matrix area around `0x06030080..0x0603017F`, not the object-source buffer. The large geometry-source values are produced later through the transform/key path (`0x0602E70C`, `0x0602E7AA`, `0x0602E7D2`, `0x0602E7EA`) and then copied into the normalizer source by `0x0602EA90`. Keep the next probe focused on the transform builder's `MAC.L`/`XTRCT` dataflow and pointer registers (`R1`, `R4`, `R7`, `R8`, `R11`, `R14`).
- The follow-up matrix probes rule out `0x0602E3B0..0x0602E40A` as the first large-value source. Parent node `0x06035264` is overwritten at `0x0602DF00`: fields `0x0603527C` and `0x06035284` change from plausible `0x00044DAF`/`0xFFF695C2` to `0x8B9510EB`/`0xDA5EA506`. The node builder consumes already-large coefficient-source values from `0x0604156C..0x06041600`; their first large writes are at `0x0602DD9C`, `0x0602DDC6`, `0x0602DE16`, `0x0602DE3E`, `0x0602DE66`, and `0x0602DE82`. VBlank critical-window deferral produces identical values. The next narrow target is therefore the unshifted input dataflow around `0x0602DD80..0x0602DE82`, not the later matrix MAC or interrupt timing.
- The root cause was missing SH-2 on-chip DIVU behavior, not the later transform arithmetic. NiGHTS helper `0x0601157C` writes divisor/high/low dividend to `0xFFFFFF00/10/14` and reads the signed quotient back from `0xFFFFFF14`. Passive internal-register storage returned the dividend low word and poisoned all downstream transforms. Signed 32/32 and 64/32 DIVU support removes the old `0x06012C84..0x06012C8A` stall: the 30M PC advances to `0x060110C4`, normalizer hits fall from about 916k to 81k, and parent-node values stay in plausible fixed-point ranges.
- After roughly 130 frames NiGHTS polls SCSP RAM command byte `0x05A00700`; a real sound 68k would clear it. `--simulate-scsp-command-ack` is an explicit bringup-only substitute. Combined with `--vblank-interval 100000`, a 50M run passes the handshake and continues through repeated returning BIOS byte-copy calls while Work RAM writes grow, so BIOS `0x00002362..0x00002368` is active copy work rather than a new infinite loop.
