...
The corrected 94M accelerated NiGHTS run consumes exactly `204/204` TOC words. It still reaches the first zero opcode at `0x06010E48`, so the old description of an unbounded TOC read was only a symptom of the invisible FIFO. The remaining blocker is earlier: the Mednafen snapshot contains code at `0x06010E48` before this point, while SystemRegis does not. Mednafen's authentication FLS itself reads the security sector into CD Block buffers and does not directly write Work RAM; the missing path must therefore be captured across the CD host-transfer/SCU-DMA/BIOS copy boundary rather than implemented as another direct executable-load shortcut.
