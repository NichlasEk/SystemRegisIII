...
The focused Mednafen execution probe found the next concrete mismatch. At entry `0x4E4`, the reference response packer observes control byte `0x24`; SystemRegis observes `0x01`. The subsequent `0x20` test therefore exits the table scan in Mednafen and continues to command `10`, while SystemRegis keeps scanning. The delayed post-session report is now emitted in the reference Pause-status layout `0100,4101,0100,00A6` instead of the earlier inferred `0100,00A6,0100,0000`; smoke coverage verifies it. A new BIOS response-stack watch covers `0x06001EBC..0x06001EDF`. With the corrected layout, the SystemRegis matcher now reaches `0x4E4` on operand `010000A6`, but its callback control byte remains `0x01` and the 94M command history still ends in `03,03`. The next reference probe must log the exact memory address/value read by `MOV.B @R4,R3` in `0x00004C50` and its preceding writer; do not synthesize `0x24` without that provenance.
