...
The remaining live-host failure was presentation state rather than texture data: WaylandForge retained the last complete VDP1 command list after the BIOS removed it, leaving an early black-background logo frame frozen past 145M instructions. `Vdp2BackScreenRenderer` now decodes the solid or per-line back-screen table from VDP2 `BKTA` and VRAM, and `Vdp1SoftwareRenderer` can composite sprites over those rows. The Wayland host uses that background every VBlank and clears stale sprites when no complete current VDP1 list exists. Integer scaling remains selected locally, but scaling was not the root cause.
