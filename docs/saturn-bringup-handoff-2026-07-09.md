...
This correction does not remove the extra status command because the BIOS wait loop branches on CMOK, not EHST. The new trace still produces `62,00,00,51,00,51,...`. It does, however, expose the next architectural difference before that command: in the scheduler helper at `0606CC40`, the task field loaded from `06066EDC` is `3` in SystemRegis but `12` in Mednafen. The reference consequently reaches task descriptor `060662A4` and command `51`, while SystemRegis first services `06066EA4` and command `00`. Trace the writers or timer source for `06066EDC` next; preserve the now matched HIRQ waveform and do not replace task selection with a command-specific shortcut.
