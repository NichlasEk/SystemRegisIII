...
## July 22 Post-Save Backup-Setup Differential

The extended 240M run proves that the repeated backup and SMPC work is finite. SystemRegis reaches the second call sequence at `0604D372..0604D37E`, returns to ordinary game code, remains fault-free, and still renders the same frame hash `9d4916c718efa8604f44ff744c2c40118ede43968d7dea4d4ac4212c9268e00e`. Backup activity remains at 46,869 reads and 16,000 writes, so the game is not continually rewriting the score file.

A clean, no-throttle Mednafen CPU probe gives an exact matched boundary. Immediately after the first helper at `060055CC`, both emulators have `R0=0`, `R1=20181100`, `R2=2010001F`, `R3=19`, `R4=48`, `R5=0`, `R6=06000354`, and `R7=40`. The following game backup setup at `0600598C` is the first concrete divergence. On its return to `0604D378`, Mednafen has `R1=23`, `R4=06062CE4`, `R5=0`, `R6=0`, and `R7=06001F1C`; SystemRegis has `R1=1`, `R4=41`, `R5=20180200`, `R6=21`, and `R7=4`. Mednafen then returns from `06005C96` with `R0=18`, `R1=2`, `R5=400`, `R6=24100000`, and `R7=1`.

Disassembly identifies the wrapper as three unconditional calls: `060055CC`, `0600598C`, and `06005C96`. Do not branch-patch this wrapper or special-case NiGHTS. A fresh-home Mednafen probe confirms that `0600598C` enumerates backup devices through the BIOS function pointer at `06062CF0 -> 0607C672`, using operation words `3100`, `3840`, and `0740`, and writes its discovered state to `060CB041..060CB04A`. The reference reaches the full three-device tail and returns `R1=23`. The next task is to trace the corresponding SystemRegis invocation through `0600598C`, especially the BIOS returns around `06005A12`, `06005A28`, and `06005AB8`, and find the first differing return or branch. This now points below the scene/object producer, in BIOS-visible backup-device enumeration or the SH-2 path executing it, rather than in VDP1, SCU DMA, the score-file layout, or a permanent SMPC wait.

The temporary instrumented Mednafen tree and all detailed traces were under `/tmp` and will not survive a reboot. Recreate the reference tree from `local/mednafen-lab` and instrument `SH7095::MemReadRT`/`MemWriteRT` for master PC range `0600598C..06005B00`; suppress repeated reads of SMPC SF address `20100063`, use a brand-new Mednafen home, and run with `-sound 0 -nothrottle 1`. The durable SystemRegis source checkpoint is pushed through `6150c2b` plus this handoff commit; the worktree was clean before this documentation update.

## July 22 Backup Memory Cartridge Enumeration

The focused SystemRegis and fresh-home Mednafen traces show that the BIOS calls at `06005A12` and `06005A28` are not themselves divergent: both return `R1=2` for operation words `3100` and `3840`. The first differing value is their side effect at `060CB041`. Mednafen reads byte `2`, continues through `06005A42`, and invokes the third `0740` operation; SystemRegis read zero from the same byte and branched directly to `06005AC8`.

The missing side effect came from real cartridge hardware, not an SH-2 branch error. Mednafen's NiGHTS profile uses its ordinary 512 KiB Backup Memory cartridge, while SystemRegis mapped `04000000..04FFFFFF` to a zero-returning A-bus stub. `SaturnBackupCartridgeBusDevice` now models the cartridge's low byte lane, 1 MiB physical mirroring, repeated `BackUpRam Format` header, and ID word `0021` at `04FFFFFE`. It is exposed by `SaturnSystemMap.BackupCartridge`, mapped through cache-through aliases, and covered by focused smoke checks.

With the cartridge present, SystemRegis reads `060CB041=2`, falls through the former early exit, and reaches the third BIOS call with `R4=1`, `R5=0740`. The cartridge scan is finite: both the 200M and 280M acceptances stop at 37,050 cartridge reads. The 280M run is fault-free and leaves the BIOS path for valid game code at master/slave PCs `0606FD08`/`06005F9E`. This hardware correction is necessary but still does not create the late VDP1 producer: the run records 1,514 automatic completions, no renewed `060DC000` command-list family, and the same eight-sprite frame hash `9d4916c718efa8604f44ff744c2c40118ede43968d7dea4d4ac4212c9268e00e`.

Continue with a post-enumeration differential of the discovered-device bytes `060CB041..060CB04A` and the return state after the full third BIOS operation, then follow the next object-manager callback. Do not remove the cartridge, branch-patch `06005A40`, or return to VDP1/SCU transfer work: the old branch is now explained and corrected, but a later game-state difference still suppresses the command-list producer.

## July 22 SMPC Bus-Buffer Readback

The full third backup operation and the second `0600598C` return now match Mednafen: SystemRegis reaches `0604D378` with `R1=23`, `R4=06062CE4`, `R5=0`, `R6=0`, and `R7=06001F1C`. The remaining `06005C96` return difference was not a backup-filesystem result. Disassembly and an instruction trace show that the routine sends SMPC command `RESENAB (19)` through `2010001F`, polls the status flag at `20100063`, and leaves that final byte read in `R0`.
