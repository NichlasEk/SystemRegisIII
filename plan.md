...
- Current slice: Added SH-2 SR.S and `MAC.L` saturation behavior with smoke coverage, improved the CLI SH-2 disassembler for indexed moves/MAC/shift/control-register opcodes, and added a geometry-source RAM watch around `0x06049E00..0x06049EFF`. The decisive Nights fix was adding the observed `0x80000000..0x9FFFFFFF` SH-2 area alias to the bus and watch normalizers: the old `Master SH-2 fault at 0x8602AF1E while executing 0x06012E54` now resolves to Work RAM High instead of faulting. Nights 80M now completes without reported bus faults or unimplemented opcodes and ends at `PC=0x06012C8A`; CD auth is still not started (`last command CR=0x0100`, command counts `0x01:1`), so the next blocker is BIOS/CD/menu progression rather than the geometry crash.
...
- The old Nights geometry crash is cleared by the `0x80000000` area alias. Latest Nights 80M run with the real CUE has no reported bus faults/unimplemented opcodes and ends at `PC=0x06012C8A`, with geometry-source watch `reads=1,526 writes=1,603`. CD Block is not yet progressing beyond hardware info/auth-startup diagnostics in that path (`auth startup completed: False`, command counts `0x01:1`).
