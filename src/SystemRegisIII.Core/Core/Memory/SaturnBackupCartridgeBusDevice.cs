...
public sealed class SaturnBackupCartridgeBusDevice : IInspectableBusDevice
{
    private const int SizeBytes = 512 * 1024;
    private const uint AddressMask = SizeBytes - 1;
    private const uint CartridgeIdOffset = 0x00FF_FFFE;
    ...
    public SaturnBackupCartridgeBusDevice()
    {
        for (var offset = 0; offset < 0x200; offset++)
        {
            _memory[offset] = FormatSignature[offset % FormatSignature.Length];
        }
    }

    public string Name => "Backup Memory Cartridge";
    ...
    public byte ReadByte(uint offset)
    {
        ...
        if ((offset & ~1u) == CartridgeIdOffset)
        {
            return (offset & 1) == 0 ? (byte)0x00 : (byte)0x21;
        }
