...
public sealed class CdBlockRegisterBusDevice : IInspectableBusDevice
{
    ...
    public void AdvanceMasterInstructions(int instructionCount)
    {
        ...
        {
            _playSeekStatusInstructionsRemaining -= instructionCount;
            if (_playSeekStatusInstructionsRemaining <= 0)
            {
        ...
        {
            _postSectorTransferStatusInstructionsRemaining -= instructionCount;
            if (_postSectorTransferStatusInstructionsRemaining <= 0)
            {
        ...
        if (_fileSystemScopeStatusInstructionsRemaining > 0)
        {
            _fileSystemScopeStatusInstructionsRemaining -= instructionCount;
            if (_fileSystemScopeStatusInstructionsRemaining <= 0)
            {
                WriteStatusResponse((byte)(_status | CdStatusPeriodic));
    ...
    private void ExecuteCommand()
    {
        ...
        else if (_discImage is not null && LastCommandCode is 0x70 or 0x71)
        {
            _hirq |= HirqEndFileSystem;
        }
    ...
    private void GetCurrentStatus()
    {
        ...
        if (_discImage is not null && _playSeekActive)
        {
    ...
    {
        _statusMode = true;
        var previousFad = _currentFad;
        var encodedStart = ((uint)(LastCommandCr1 & 0x00FF) << 16) | LastCommandCr2;
        var startFad = (encodedStart & 0x80_0000) != 0
            ? encodedStart & 0x7F_FFFF
            : FirstTrackFad;
        var encodedEnd = ((uint)(LastCommandCr3 & 0x00FF) << 16) | LastCommandCr4;
        var sectorCount = (encodedStart & 0x80_0000) != 0 && (encodedEnd & 0x80_0000) != 0
            ? Math.Max(1, (int)(encodedEnd & 0x7F_FFFF))
    ...
    private void PublishPlaySeekStatus()
    {
    ...
    private void ChangeDirectory()
    {
        ...
        EnsureFileInfosLoaded();
        if (LastCommandCr3 == 0x00FF && LastCommandCr4 == 0xFFFF)
        {
            _rootDirectoryLoadPending = true;
            _statusMode = true;
            _cr1 = 0x0180;
            _cr2 = 0x4101;
            _cr3 = 0x0100;
    ...
    private void GetFileSystemScope()
    {
        ...
        _cr3 = 0x0100;
        _cr4 = 0x0002;
        if (_rootDirectoryLoadPending)
        {
            _rootDirectoryLoadPending = false;
            _fileSystemScopeStatusInstructionsRemaining = 2_000;
        }
    }
